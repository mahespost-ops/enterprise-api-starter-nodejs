 ...
 * Tests follow TDD pattern - written first, then adapters are implemented to pass.
 */

import { ISecretsAdapter, ICreateSecretParams } from '../secrets.interface';

/**
 * Generic test suite that works with any ISecretsAdapter implementation
 */
export function testSecretsAdapter(
  adapterName: string,
  createAdapter: () => Promise<ISecretsAdapter>
): void {
  describe(`${adapterName} Secrets Adapter`, () => {
    ...
    describe('Error Handling', () => {
      ...
      it('should throw error when deleting non-existent secret', async () => {
        await expect(
          adapter.deleteSecret('does-not-exist-delete-xyz')
        ).rejects.toThrow();
      });
    ...
    describe('Versioning Support', () => {
      ...
      it('should include version in metadata', async () => {
        ...
        const result = await adapter.getSecretWithMetadata(
          'test-secret-version-meta'
        );
...
export const testUtils = {
  generateRandomSecretName: () => `test-${Date.now()}-${Math.random().toString(36).substring(7)}`,

  generateTestSecret: (name?: string) => ({
    name: name || testUtils.generateRandomSecretName(),
    value: `secret-${Math.random().toString(36).substring(7)}`,
  }),
};
